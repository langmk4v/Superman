...
namespace superman {
  ...
  public:
    Parser(Lexer& x, std::vector<Token>& _tokens)
        : lexer(x), source(x._source), tokens(_tokens), cur(tokens.data()) {
      (void)lexer;
      (void)source;
      (void)tokens;
    }
    ...
    NdSymbol* ps_type_name() { return ps_symbol(true); }
