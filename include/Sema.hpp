...
struct SCClass : Scope {
  ...
  NdClass* get_node() { return node->as<NdClass>(); }
...
struct NdVisitorContext {
  ...
  static std::string ctx2s(NdVisitorContext ctx) {
    ...
    if (ctx.empty_array_element_type)
      flags +=
          format("empty_array_element_type: %p,", ctx.empty_array_element_type);
    if (ctx.can_use_empty_array)
      flags += format("can_use_empty_array: %d,", ctx.can_use_empty_array);
    if (ctx.slice_target_array_type)
      flags +=
          format("slice_target_array_type: %p,", ctx.slice_target_array_type);
    if (ctx.loop_depth) flags += format("loop_depth: %d,", ctx.loop_depth);
    if (ctx.expected_type)
      flags += format("expected_type: %p,", ctx.expected_type);
    if (ctx.as_typename) flags += format("as_typename: %d,", ctx.as_typename);
    if (ctx.as_callee_of_callfunc)
      flags += format("as_callee_of_callfunc: %d,", ctx.as_callee_of_callfunc);
    if (ctx.as_arg_of_callfunc)
      flags += format("as_arg_of_callfunc: %d,", ctx.as_arg_of_callfunc);
    if (ctx.parent_cf_nd)
      flags += format("parent_cf_nd: %p,", ctx.parent_cf_nd);
    if (ctx.self_ty_ptr) flags += format("self_ty_ptr: %p,", ctx.self_ty_ptr);
    if (ctx.enumerator_node_out)
      flags += format("enumerator_node_out: %p,", ctx.enumerator_node_out);
    return "{" + flags + "}";
  }
#endif
};
...
public:
  ...
  ArgumentsCompareResult compare_arguments(NdCallFunc* cf, NdFunction* fn,
