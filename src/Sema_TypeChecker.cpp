...
namespace fire {
  ...
  TypeChecker::compare_arguments(NdCallFunc* cf, NdFunction* fn, BuiltinFunc const* builtin,
                                 bool is_var_arg, bool is_method_call, TypeInfo& self_ty,
                                 std::vector<TypeInfo> const& defs, std::vector<TypeInfo>& actual) {
    (void)cf;
    (void)fn;
    (void)builtin;
    (void)is_var_arg;
    ...
    for (size_t i = 0; i < defs.size(); i++) {
      if (defs[i].kind == TypeKind::Any) { continue; }
    ...
    if (cf->is_method_call) {
      ...
      if (self_ty.is(TypeKind::Class)) {
        ...
        for (NdFunction* method : nd_class->methods) {
          if (method->name.text == method_name) {
            ...
            for (auto arg : method->args) { arg_defs.push_back(eval_typename_ty(arg.type, ctx)); }
            auto cmp = compare_arguments(cf, method, nullptr, method->is_var_args, true, self_ty,
                                         arg_defs, arg_types);
            ...
            if (method->result_type) cf->ty = this->eval_typename_ty(method->result_type, ctx);
      ...
      for (BuiltinFunc const* method : builtin_method_table) {
        if (method->name == method_name && method->self_type.kind == self_ty.kind) {
          auto cmp = compare_arguments(cf, nullptr, method, method->is_var_args, true, self_ty,
      ...
      throw err::e(cf->callee->token,
                   "method '" + method_name + "' not found in '" + self_ty.to_string() + "'");
    }
    ...
    if (callee_ty.is(TypeKind::Class)) {
      ...
      if (argc_give > nd_class->fields.size()) {
        throw err::e(cf->args[argc_give]->token,
                     "too many arguments for constructor of '" + callee_ty.to_string() + "'");
      } else if (argc_give < nd_class->fields.size()) {
        throw err::e(cf->args[argc_give]->token,
                     "too few arguments for constructor of '" + callee_ty.to_string() + "'");
      }
      for (size_t i = 0; i < argc_give; i++) {
        ...
        if (kvp->key->token.text != nd_class->fields[i]->name.text) {
          throw err::e(kvp->key->token, "field '" + kvp->key->token.text + "' is not found in '" +
                                            callee_ty.to_string() + "'");
        }
        // check type matching
  ...
  TypeInfo TypeChecker::case_construct_enumerator(NdCallFunc* cf, NdEnumeratorDef* en_def,
    ...
    // one variant
      ...
      if (argc_give == 0) {
        ...
        if (!arg_types[0].equals(eval_expr_ty(en_def->variant, ctx))) {
          throw err::mismatched_types(cf->args[0]->token,
        ...
        for (size_t i = 0; i < count; i++) {
          auto expected_ty =
              en_def->multiple[i]->is(NodeKind::Symbol)
                  ...
                  : eval_typename_ty(
                        en_def->multiple[i]->as<NdKeyValuePair>()->value->as<NdSymbol>(), ctx);
...
#endif
    switch (node->kind) {
      case NodeKind::Value: node->ty = node->as<NdValue>()->obj->type; break;
      case NodeKind::Symbol: {
        ...
        switch (sym->symbol_ptr->kind) {
          ...
          case SymbolKind::TemplateParam: sym->ty = eval_typename_ty(sym, ctx); break;
          case SymbolKind::Var:
          ...
          case SymbolKind::Func: {
            ...
            node->ty.parameters = {
                fn_node->result_type ? eval_typename_ty(fn_node->result_type, ctx) : TypeInfo()};
          ...
          case SymbolKind::Namespace: todo;
          case SymbolKind::Module: todo; // !?!?
          case SymbolKind::BuiltinFunc: node->ty = sym->symbol_ptr->type; break;
          default: todo;
        }
        break;
      }
      case NodeKind::KeyValuePair: {
        ...
        if (ctx.as_arg_of_callfunc) { return eval_expr_ty(kvp->value, ctx); }
        todo;
      }
      case NodeKind::Self: {
      ...
      case NodeKind::Array: {
        ...
        for (auto& elem : arr->data) { elems.push_back(eval_expr_ty(elem, ctx)); }
        if (elems.empty()) {
      ...
      case NodeKind::Tuple: {
        ...
        std::vector<TypeInfo> elems;
        for (auto& elem : tuple->elems) { elems.push_back(eval_expr_ty(elem, ctx)); }
      ...
      case NodeKind::MemberAccess: {
        ...
        throw err::e(mm->rhs->token,
                     "field '" + field_name + "' not found in '" + obj_ty.to_string() + "'");
      }
    ...
    switch (node->kind) {
      case NodeKind::Symbol: {
        ...
        switch (sym->symbol_ptr->kind) {
          ...
          case SymbolKind::Class: return make_class_type(sym->symbol_ptr->node->as<NdClass>());
          case SymbolKind::BuiltinType: {
            TypeInfo ty = sym->symbol_ptr->type;
            for (auto p : sym->te_args) { ty.parameters.push_back(eval_expr_ty(p, ctx)); }
            switch (ty.kind) {
          ...
          case SymbolKind::TemplateParam: todo;
          default: err::emitters::expected_type_name_here(node->token);
        }
        break;
      }
      default: todo;
  ...
  void TypeChecker::check_stmt(Node* node, NdVisitorContext ctx) {
    switch (node->kind) {
      ...
      case NodeKind::Try: {
        ...
        if (nd_try->finally_block) { check_scope(nd_try->finally_block, ctx); }
        todo;
      }
      case NodeKind::Break: {
        todo;
  ...
  void TypeChecker::check_scope(NdScope* node, NdVisitorContext ctx) {
    ...
    for (auto& item : node->items) {
      switch (item->kind) {
        case NodeKind::Scope: check_scope(item->as<NdScope>(), ctx); break;
        default: check_stmt(item, ctx); break;
      }
    }
  }
  ...
  void TypeChecker::check_class(NdClass* node, NdVisitorContext ctx) {
    ...
    for (auto field : node->fields) { check_stmt(field, ctx); }
  ...
  void TypeChecker::check_namespace(NdNamespace* node, NdVisitorContext ctx) {
    ...
    for (auto& item : node->items) {
      switch (item->kind) {
        case NodeKind::Let: check_stmt(item, ctx); break;
        case NodeKind::Function: check_function(item->as<NdFunction>(), ctx); break;
        case NodeKind::Class: check_class(item->as<NdClass>(), ctx); break;
        case NodeKind::Enum: check_enum(item->as<NdEnum>(), ctx); break;
        case NodeKind::Namespace: check_namespace(item->as<NdNamespace>(), ctx); break;
        default: todo;
      }
    }
  }
