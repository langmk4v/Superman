...
namespace fire {
  ...
  void NameResolver::on_expr(Node* node, NdVisitorContext ctx) {
    ...
    switch (node->kind) {
      case NodeKind::Symbol: {
        ...
        for (auto template_param : sym->te_args) on_typename(template_param, ctx);
        break;
      }
      ...
      case NodeKind::Array: {
        ...
        for (auto& item : arr->data) on_expr(item, ctx);
        break;
      }
      case NodeKind::Tuple: {
        auto tuple = node->as<NdTuple>();
        for (auto& item : tuple->elems) on_expr(item, ctx);
        break;
      }
      case NodeKind::Slice: {
      ...
      case NodeKind::CallFunc: {
        ...
        if (call->is_method_call) { on_expr(call->inst_expr, ctx); }
        on_expr(call->callee, ctx);
        for (auto& arg : call->args) on_expr(arg, ctx);
        break;
      }
      ...
      case NodeKind::New: {
        ...
        on_expr(x->type, ctx);
        for (auto& arg : x->args) on_expr(arg, ctx);
        break;
      }
      case NodeKind::Mul:
      case NodeKind::Div:
  ...
  void NameResolver::on_scope(Node* node, NdVisitorContext ctx) {
    ...
    for (auto& item : scope->items) { on_stmt(item, ctx); }
  }
  void NameResolver::on_function(Node* node, NdVisitorContext ctx) {
    ...
    for (auto arg : func->args) { on_typename(arg.type, ctx); }
  ...
  void NameResolver::on_class(Node* node, NdVisitorContext ctx) {
    ...
    for (auto& field : C->fields) { on_stmt(field, ctx); }
    for (auto& method : C->methods) { on_function(method, ctx); }
  }
    ...
    for (auto& en : E->enumerators) {
      switch (en->type) {
        ...
        case NdEnumeratorDef::StructFields:
          for (auto& i : en->multiple)
            on_typename(i->as<NdKeyValuePair>()->value->as<NdSymbol>(), ctx);
          break;
        case NdEnumeratorDef::OneType: on_typename(en->variant, ctx);
      }
    }
  }
  void NameResolver::on_namespace(Node* node, NdVisitorContext ctx) {
    ...
    for (auto& item : NS->items) {
      switch (item->kind) {
        case NodeKind::Let: on_stmt(item, ctx); break;
        case NodeKind::Function: on_function(item, ctx); break;
        case NodeKind::Class: on_class(item, ctx); break;
        case NodeKind::Enum: on_enum(item, ctx); break;
        case NodeKind::Namespace: on_namespace(item, ctx); break;
      }
    }
  }
