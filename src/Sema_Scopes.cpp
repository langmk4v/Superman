...
namespace fire {
  Scope* Scope::from_node(Node* node, Scope* parent) {
    switch (node->kind) {
      case NodeKind::Scope: return new SCScope(node->as<NdScope>(), parent);
      case NodeKind::If: return new SCIf(node->as<NdIf>(), parent);
      case NodeKind::For: return new SCFor(node->as<NdFor>(), parent);
      case NodeKind::While: return new SCWhile(node->as<NdWhile>(), parent);
      case NodeKind::Loop:
        todo;
        // return new SCLoop(node->as<NdLoop>(), parent);
      case NodeKind::Catch: return new SCCatch(node->as<NdCatch>(), parent);
      case NodeKind::Try: return new SCTry(node->as<NdTry>(), parent);
      case NodeKind::Function: return new SCFunction(node->as<NdFunction>(), parent);
      case NodeKind::Enum: return new SCEnum(node->as<NdEnum>(), parent);
      case NodeKind::Class: return new SCClass(node->as<NdClass>(), parent);
      case NodeKind::Namespace: return new SCNamespace(node->as<NdNamespace>(), parent);
      case NodeKind::Module: return new SCModule(node->as<NdModule>(), parent);
      default: todo;
    }
    todo;
  }
  ...
  SCScope::SCScope(NdScope* node, Scope* parent) : Scope(ScopeKind::Scope, node, parent) {
    ...
    for (auto item : node->items) {
      switch (item->kind) {
        ...
        case NodeKind::Scope: subscopes.push_back(new SCScope(item->as<NdScope>(), this)); break;
        case NodeKind::If:
        case NodeKind::Match:
        case NodeKind::For:
        case NodeKind::While:
        case NodeKind::Loop:
        case NodeKind::Try: subscopes.push_back(Scope::from_node(item, this)); break;
      }
    }
  }
