...
namespace fire {
  ...
  ExprType Sema::eval_expr(Node* node) {
    ...
    case NodeKind::Symbol: {
      ...
      if (result.count() >= 2) throw err::ambiguous_symbol_name(sym->name);
      ...
      if (result.count()) {
        ...
        case SymbolKind::Var: {
          ...
          debug(if (S->var_info->is_argument()) assert(S->var_info->is_type_deducted);)
        ...
        case SymbolKind::Func: {
          ...
          auto ti = TypeInfo(TypeKind::Function, {f->result_type ? eval_typename(f->result_type).type : TypeKind::None},
                             false, false);
    ...
    case NodeKind::CallFunc: {
      ...
      if (callee.func_nd) cf->func_nd = callee.func_nd;
    ...
    switch (ex->kind) {
    case NodeKind::Add:
      if (left.is(TypeKind::String) && right.is(TypeKind::String)) return {node, TypeInfo(TypeKind::String)};
      break;
    ...
    if (left.is_numeric() && right.is_numeric()) {
      return {node, TypeInfo(left.is(TypeKind::Float) || right.is(TypeKind::Float) ? TypeKind::Float : TypeKind::Int)};
    }
