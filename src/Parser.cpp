...
namespace fire {
  NdSymbol* Parser::ps_symbol(bool as_typename) {
    ...
    if (eat("<")) {
      try {
        do sym->te_args.emplace_back(ps_type_name());
        while (!is_end() && eat(","));
        if (look(">>"))
          cur->text = ">";
        else
  ...
  NdSymbol* Parser::ps_type_name() {
    ...
    if (eat("(")) {
      ...
      expect(",");
      do { tuple_type->te_args.push_back(ps_type_name()); } while (!is_end() && eat(","));
      expect(")");
      return tuple_type;
    }
    if (eat("decltype")) {
  ...
  Node* Parser::ps_factor() {
    ...
    if (eat("(")) {
      ...
      if (eat(",")) {
        ...
        do { tu->elems.push_back(ps_expr()); } while (!is_end() && eat(","));
        node = tu;
      }
      expect(")");
      return node;
    ...
    if (eat("[")) {
      ...
      if (eat("]")) return node;
      do { node->data.emplace_back(ps_expr()); } while (eat(","));
      expect("]");
      return node;
    }
    ...
    if (eat("decltype")) { throw err::parses::cannot_use_decltype_here(tok); }
    if (cur->is(TokenKind::Identifier)) { return ps_symbol(); }
    ...
    switch (cur->kind) {
      ...
      case TokenKind::Char: {
        ...
        if (s16.empty() || s16.size() > 1) throw err::invalid_character_literal(*cur);
        v->obj = new ObjChar(s16[0]);
        next();
        break;
      }
      case TokenKind::String: {
        v->obj = ObjString::from_char16_ptr_move(
            utf8_to_utf16_with_len(nullptr, cur->text.data() + 1, cur->text.length() - 2));
        next();
        break;
      }
  ...
  Node* Parser::ps_subscript() {
    ...
    if (eat("++")) { return new NdInclement(tok, x, false); }
    if (eat("--")) { return new NdDeclement(tok, x, false); }
    return x;
  }
  Node* Parser::ps_unary() {
    auto& tok = *cur;
    if (eat("++")) { return new NdInclement(tok, ps_subscript(), true); }
  ...
  Node* Parser::ps_log_and() {
    auto x = ps_bit_or();
    while (!is_end() && eat("&&")) x = new NdExpr(NodeKind::LogAnd, *cur->prev, x, ps_bit_or());
    return x;
  }
  Node* Parser::ps_log_or() {
    auto x = ps_log_and();
    while (!is_end() && eat("||")) x = new NdExpr(NodeKind::LogOr, *cur->prev, x, ps_log_and());
    return x;
  }
  ...
  NdLet* Parser::ps_let(bool expect_semi) {
    ...
    if (eat("(")) {
      do { let->placeholders.push_back(expect_ident()); } while (!is_end() && eat(","));
      expect(")");
    } else
      expect_ident();
  ...
  Node* Parser::ps_stmt() {
    ...
    if (eat("try")) {
      ...
      x->body = ps_scope();
      if (!look("catch")) { throw err::parses::expected_catch_block(*cur->prev); }
      ...
      if (eat("finally")) { x->finally_block = ps_scope(); }
      return x;
    }
    if (eat("if")) {
      auto x = new NdIf(tok);
  ...
  NdClass* Parser::ps_class() {
    ...
    while (!is_end()) {
      ...
      // field
        ...
        if (node->m_new) throw err::duplicate_of_definition(*cur->prev, node->m_new->token);
        ...
        if (look("->")) { throw err::cannot_specify_return_type_of_constructor(*cur); }
        newfn->body = ps_scope();
        node->m_new = newfn;
      } else if (eat("delete")) {
        todoimpl;
  ...
  NdEnumeratorDef* Parser::ps_enumerator_def() {
    ...
    if (eat("(")) {
      ...
      if (eat(",")) {
        ...
        do { nd->multiple.emplace_back(ps_type_name()); } while (!is_end() && eat(","));
        expect(")");
        return nd;
      }
  ...
  NdModule* Parser::ps_mod() {
    ...
    while (!is_end() && eat("import")) { ps_import(); }
    for (auto&& src : source.imports) {
      if (src->is_node_imported) continue;
      auto submod = src->parse();
      for (auto&& item : submod->items) { mod->items.emplace_back(item); }
      src->is_node_imported = true;
    }
    while (!is_end()) {
      if (look("import")) { throw err::parses::import_not_allowed_here(*cur); }
      ...
      if (auto orig = items[i]->as<NdNamespace>(); orig->is(NodeKind::Namespace)) {
        for (size_t j = i + 1; j < items.size(); j++) {
          if (auto dup = items[j]->as<NdNamespace>();
            ...
            for (auto x : dup->items) orig->items.push_back(x);
            delete dup;
