...
std::string node2s(Node* node) {
  ...
  case NodeKind::Deref: {
    ...
    return x->token.text + node2s(x->expr);
  }
  ...
  case NodeKind::Let: {
    ...
    if (x->placeholders.size() >= 1) {
      ss << "("
         << join(", ", x->placeholders, [](Token const& t) { return t.text; })
         << ")";
    } else
      ss << x->name.text;
    if (x->type) {
  ...
  case NodeKind::If: {
    auto x = node->as<NdIf>();
    std::stringstream ss;
    ss << "if " << node2s(x->vardef) << node2s(x->cond) << " "
       << node2s(x->thencode);
    if (x->elsecode)
  ...
  case NodeKind::For: {
    ...
    std::stringstream ss;
    ss << "for " << x->iter.text << " " << node2s(x->iterable) << " "
       << node2s(x->body);
    return ss.str();
  }
  ...
  case NodeKind::Function: {
    ...
    auto args = join(", ", x->args, [](NdFuncArgument* arg) {
      return arg->name.text + ": " + node2s(arg->type);
    });

    if (x->take_self) {
      args = "self"s + (args.empty() ? "" : ", ") + args;
    }

    ss << "(" << args << ") -> " << node2s(x->result_type) << " "
       << node2s(x->body);
  ...
  case NodeKind::EnumeratorDef: {
    ...
    if (x->type == NdEnumeratorDef::NoVariants) {
      ...
      ss << x->name.text << "(";
      if (x->type == NdEnumeratorDef::MultipleTypes ||
          x->type == NdEnumeratorDef::StructFields) {
